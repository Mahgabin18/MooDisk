...
const Online = memo((props) => {
  ...
  const seePlaylists = (e) => {
    document.getElementById("PopUp").style.marginTop =
      e.currentTarget.id + "px";
    if (
      !open ||
      (addingTrack.current && addingTrack.current === e.currentTarget.value)
    ) {
      setOpen(!open);
    }
  ...
  return onlineClicked && props.songs ? (
    <div className="AboveOnline">
      <div className="Online">
        ...
        {typeof props.songs === "object" ? (
          props.songs.tracks.items.map((item, idx) => (
            <div className="AddParent" key={idx}>
              <button
                id={idx * 23 + 57}
                value={item.uri}
              ...
              >
                ...
                <div className="AddMessage">&nbsp;Add to playlist</div>
              </button>
      ...
      <div id="PopUp" className="PopUp">
        {open &&
          props.playlists.current.listOfPlaylistsFromAPI.map(
            (item, idx) =>
              userInfo.current.display_name === item.owner.display_name && (
                <button
                  className="AddToPlay"
                  key={idx}
                  id={item.id}
